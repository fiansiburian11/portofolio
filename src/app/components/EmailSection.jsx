...
const EmailSection = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const data = {
      email: e.target.email.value, // Email dari pengguna
      subject: e.target.subject.value, // Subjek dari form
      message: e.target.message.value, // Pesan dari form
    };
    const JSONdata = JSON.stringify(data);
    const endpoint = "http://localhost:3000/api/send"; // Endpoint API untuk mengirim email

    // Form the request for sending data to the server
    const options = {
      method: "POST", // POST karena kita mengirim data
      headers: {
        "Content-Type": "application/json", // Menggunakan JSON
      },
      body: JSONdata, // Body berisi data JSON
    };
    ...
    if (response.status === 200) {
      ...
      setEmailSubmitted(true); // Mengubah status ketika email berhasil dikirim
    }
  };
